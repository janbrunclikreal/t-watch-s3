...
async def hlidac_korunky_task():
    ...
    def direct_read_reg(reg):
        # Zkontrolujeme, jestli se podařilo zamknout, hned na vstupu
        if not i2c.try_lock():
            return 0
        ...
        try:
            ...
            # Tady můžete případně přidat logování chyby, 
            # pokud chcete vidět, že čtení selhalo
            print(f"I2C read error: {e}")
            return 0
        finally:
            # Zámek se uvolní VŽDY, jakmile proběhne try nebo except blok
            i2c.unlock()
    ...
    def direct_write_reg(reg, val):
            try:
                if i2c.try_lock():
                    ...
                    # Zde můžete případně zalogovat, že se nepodařilo získat zámek
                    pass
            except Exception as e:
                # Tichá chyba je pryč, zde zachytíte konkrétní selhání zápisu
...
async def graficka_smycka_hodin_task():
    """Obnovování textů ciferníku a diagnostiky CPU - optimalizované"""
        ...
        if displej_vzhuru and rtc_hw:
            try:
                ...
                if current_state == STATE_WATCHFACE:
                    ...
                    # Aktualizaci RAM, baterie a I2C netaháme každých 0.2s, 
                    # ale rozdělíme to na delší intervaly, ať to nezatěžuje sběrnici
                    if t.tm_sec % 5 == 0:
            ...
            # Důležité: await sleep musí být vždy, aby task pustil procesor ostatním
            await asyncio.sleep(0.2)
        else:
            # Display je OFF - spíme déle, ať procesor nic nedělá
            await asyncio.sleep(0.5)
